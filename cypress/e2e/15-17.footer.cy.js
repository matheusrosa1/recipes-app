...
describe('15 - Implemente o menu inferior posicionando-o de forma fixa e contendo 2 ícones: um para comidas e outro para bebidas', () => {
  ...
  it('Verifica a cobertura de 45% do Componente Footer', () => {
    cy.getCoverage().its('Footer.functions.pct', { timeout: 0 }).should('be.gte', 45.00);
    cy.getCoverage().its('Footer.branches.pct', { timeout: 0 }).should('be.gte', 45.00);
  });
});

describe('16 - Exiba o menu inferior apenas nas telas indicadas pelo protótipo', () => {
  const hasNoFooter = () => {
...
describe('17 - Redirecione a pessoa usuária para a tela correta ao clicar em cada ícone no menu inferior', () => {
  ...
  it('Verifica a cobertura de 90% do Componente Footer', () => {
    cy.getCoverage().its('Footer.functions.pct', { timeout: 0 }).should('be.gte', 90.00);
    cy.getCoverage().its('Footer.branches.pct', { timeout: 0 }).should('be.gte', 90.00);
  });
